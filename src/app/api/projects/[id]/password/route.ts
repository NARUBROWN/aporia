...
import { requestHasOwnedProjectAccess, requestIsProjectOwner } from "@/lib/auth";
...
export async function POST(request: Request, context: RouteContext<"/api/projects/[id]/password">) {
  ...
  if (current.ownerId
    ? !await requestIsProjectOwner(request, current.ownerId)
    : !await requestHasOwnedProjectAccess(request, id, current.ownerId, current.passwordHash))
